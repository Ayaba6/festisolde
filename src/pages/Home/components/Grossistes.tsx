  ...
  return (
    <div className="min-h-screen bg-[#F8F9FB] pb-20 font-sans">
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-end justify-between gap-6">
          ...
          <div className="max-w-md w-full relative">
            ...
            <input 
              ...
              className="w-full pl-14 pr-6 py-5 bg-slate-100 border-none rounded-[1.5rem] text-xs font-bold uppercase outline-none focus:ring-2 focus:ring-orange-500 transition-all"
              value={searchTerm}
      ...
      <main className="max-w-7xl mx-auto px-6 py-12">
          ...
          {filteredStores.map((store) => (
            ...
            >
              ...
              <div className="px-8 pb-8 flex-1">
                ...
                <div className="mb-6">
                  <h2 className="text-xl font-black uppercase tracking-tighter text-slate-900 group-hover:text-orange-600 transition-colors">{store.name}</h2>
                ...
                <p className="text-[11px] text-slate-500 font-medium leading-relaxed mb-8 line-clamp-2">
                  {store.description || "Fournisseur spécialisé dans l'import-export et la vente de gros lots certifiés Festisolde."}
                </p>

                {/* Stats de la boutique */}
                <div className="flex items-center justify-between py-4 border-t border-slate-50">
                  <div className="flex flex-col">
                    <span className="text-[9px] font-black text-slate-300 uppercase">Arrivages</span>
                    <span className="text-sm font-black italic">12+ Lots</span>
                  </div>
                ...
                <button className="w-full mt-6 py-4 bg-slate-50 group-hover:bg-black group-hover:text-white rounded-2xl flex items-center justify-center gap-3 transition-all duration-300">
                  <span className="text-[10px] font-black uppercase tracking-widest">Voir le catalogue</span>
        ...
        {filteredStores.length === 0 && (
          <div className="py-40 text-center">
            ...
            <p className="text-xs font-black uppercase text-slate-400 tracking-widest">Aucun grossiste trouvé</p>
          </div>
        )}
      </main>
      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="bg-orange-600 rounded-[3rem] p-12 relative overflow-hidden flex flex-col md:flex-row items-center justify-between gap-8 shadow-2xl shadow-orange-600/20">
          <div className="relative z-10 text-center md:text-left">
            <h2 className="text-3xl md:text-5xl font-black text-white uppercase italic tracking-tighter leading-none mb-4">
              Vous êtes <br /> <span className="text-black">Fournisseur ?</span>
            </h2>
            <p className="text-orange-100 text-[10px] font-bold uppercase tracking-[0.2em]">Liquidez vos stocks en 24h auprès de notre réseau d'acheteurs.</p>
          </div>
          <button 
          ...
          >
            <Truck size={16} /> Ouvrir mon entrepôt
          </button>
          
          <div className="absolute -top-10 -right-10 w-64 h-64 bg-orange-500 rounded-full blur-3xl opacity-50"></div>
        </div>
      </div>
    </div>
  );
...
function LoadingScreen() {
  return (
    <div className="h-screen flex flex-col items-center justify-center bg-white">
      ...
      <span className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Recherche des partenaires...</span>
    </div>
  );
}