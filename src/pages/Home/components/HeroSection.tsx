...
export default function LiquidationHero() {
    ...
    async function fetchLiquidation() {
      try {
        ...
        const { data, error } = await supabase
          .from('products')
          .select('*')
          .ilike('category', '%liquidation%') // 🔹 filtrer par catégorie
          .gt('stock', 0)
          .limit(8)
        ...
        // Nettoyage : ne garder que les produits avec images
        const cleanProducts = data.filter(p => p.images && p.images.length > 0)
        setProducts(cleanProducts)
      } catch (err: any) {
        console.error('Erreur LiquidationHero:', err.message)
  ...
  if (loading) return <div className="text-center py-16">Chargement des produits...</div>
  if (error) return <div className="text-center py-16 text-red-500">Impossible de charger les produits: {error}</div>
    ...
    <section className="relative bg-[#050505] text-white border-b border-white/5 overflow-hidden font-sans">
      ...
      <style>{`
        .swiper-button-next, .swiper-button-prev { 
          ...
          color: white !important; top: 50% !important; visibility: hidden; 
        }
        @media (min-width: 1024px) { .swiper-button-next, .swiper-button-prev { visibility: visible; } }
